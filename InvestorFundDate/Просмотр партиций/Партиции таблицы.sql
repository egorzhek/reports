-- смотрим партиции таблицы - распределено по файловым группам
SELECT
...
WHERE sc.name = 'dbo' and so.name = 'InvestorFundDate'
and si.type_desc = 'CLUSTERED'
ORDER BY [Schema.Table], [Index ID], [Partition Function], [Partition #];

/*
SELECT top 100 * FROM InvestorFundDate nolock
WHERE $PARTITION.YEAR_Partition_Function([Date]) = 13
order by [Date]
*/